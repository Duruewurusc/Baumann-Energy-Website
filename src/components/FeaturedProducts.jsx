...
import { FiArrowRight, FiArrowLeft, FiSun, FiBattery, FiZap, FiCpu, FiBox, FiWind } from "react-icons/fi";
...
const FeaturedProducts = () => {
  ...
  const scroll = (direction) => {
    if (scrollContainerRef.current) {
      ...
      const cardWidth = window.innerWidth >= 1024 ? 380 : window.innerWidth >= 640 ? 350 : 300;
  ...
  return (
    <section className="w-full bg-linear-to-b from-gray-50 to-white py-15 px-4 md:px-8 lg:px-16 xl:px-24 relative">
      {/* Decorative Elements */}
      ...
      >
        ...
        <div className="hidden lg:flex absolute -left-4 top-1/2 transform -translate-y-1/2 z-20">
          <button
            onClick={() => scroll('left')}
            className="bg-white/90 backdrop-blur-sm hover:bg-white text-gray-800 rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 group"
            aria-label="Scroll left"
          >
            <FiArrowLeft className="w-6 h-6 text-green-600 group-hover:-translate-x-1 transition-transform duration-300" />
          </button>
        </div>
        ...
        <div className="hidden lg:flex absolute -right-4 top-1/2 transform -translate-y-1/2 z-20">
          ...
          >
            <FiArrowRight className="w-6 h-6 text-green-600 group-hover:translate-x-1 transition-transform duration-300" />
          </button>
        </div>
        ...
        <div 
          ...
          className="overflow-x-auto overflow-y-visible pb-8 hide-scrollbar scroll-smooth"
        >
            ...
            {products.map((product, index) => {
              ...
              return (
                <motion.article
                  ...
                  whileHover={{ y: -8 }}
                  className="group relative bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden cursor-pointer w-75 sm:w-87.5 lg:w-95 shrink-0"
                >
      ...
      >
        <motion.a
          ...
          whileTap={{ scale: 0.95 }}
          className="inline-flex items-center gap-2 bg-linear-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white px-8 py-4 rounded-full font-semibold tracking-wide shadow-lg hover:shadow-xl transition-all duration-300 group"
        >
